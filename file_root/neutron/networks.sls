...
openstack_network_{{ network }}:
  neutron:
    ...
    - connection_password: {{ salt['pillar.get']('keystone:tenants:%s:users:%s:password' % (salt['pillar.get']('neutron:networks:%s' % network).get('tenant', 'admin'), salt['pillar.get']('neutron:networks:%s' % network).get('user', 'admin')), 'admin_pass') }}
