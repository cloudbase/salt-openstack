...
{% set keystone_admin_endpoint = keystone['openstack_services']['keystone']['endpoint']['adminurl'].format(openstack_parameters['controller_ip']) %}
{% set keystone_admin_token = salt['ini.get_option'](keystone['conf']['keystone'], "DEFAULT", "admin_token") %}
...
{% set openstack_services = salt['openstack_utils.os_services']('controller') %}
...
{% if all_services_running == True %}
        ID=`cinder --os-username={{ temporary_admin_name }} --os-tenant-name={{ temporary_tenant_name }} --os-password={{ temporary_admin_password }} --os-auth-url={{ keystone_admin_endpoint }} --os-volume-api-version=2 list --all-tenants | awk '{if (NR == 4){print $2}}'`
        while [ "$ID" != "" ]; do
            cinder --os-username={{ temporary_admin_name }} --os-tenant-name={{ temporary_tenant_name }} --os-password={{ temporary_admin_password }} --os-auth-url={{ keystone_admin_endpoint }} --os-volume-api-version=2 force-delete $ID
            while [ "`cinder --os-username={{ temporary_admin_name }} --os-tenant-name={{ temporary_tenant_name }} --os-password={{ temporary_admin_password }} --os-auth-url={{ keystone_admin_endpoint }} --os-volume-api-version=2 list --all-tenants | grep $ID`" != "" ]; do
                sleep 1
            done
            ID=`cinder --os-username={{ temporary_admin_name }} --os-tenant-name={{ temporary_tenant_name }} --os-password={{ temporary_admin_password }} --os-auth-url={{ keystone_admin_endpoint }} --os-volume-api-version=2 list --all-tenants | awk '{if (NR == 4){print $2}}'`
        done
